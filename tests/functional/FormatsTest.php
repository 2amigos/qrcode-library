...
class FormatsTest extends TestCase
{
    public function testBookMark() {
        ...
        $this->assertEquals("MEBKM:TITLE:test-title;URL:http://2amigos.us;;", $bookmark->getText());

        $this->setExpectedException('yii\base\InvalidConfigException');
        $bookmark = new BookMark(['url' => 'wrongurl']);
    }
    ...
    public function testVCardNoEmail() {
        $vcard = new vCard();

        $class = new \ReflectionClass('dosamigos\\qrcode\\formats\\vCard');
        $method = $class->getMethod('getFormattedEmail');
        $method->setAccessible(true);
